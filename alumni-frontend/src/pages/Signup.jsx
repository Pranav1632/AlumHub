...
import { FiArrowRight, FiCheckCircle, FiRefreshCw, FiUserPlus } from "react-icons/fi";
...
export default function Signup() {
  ...
  const signupPayload = useMemo(
    () => ({
      ...
      skills: splitList(formData.skillsText),
      interests: splitList(formData.interestsText),
      achievements: splitList(formData.achievementsText),
    }),
    [formData]
  );
  ...
  return (
    <div className="min-h-[80vh] flex items-center justify-center px-4 py-6">
      <div className="w-full max-w-3xl bg-white border border-slate-200 shadow-lg rounded-2xl p-6 md:p-8">
        ...
        <p className="text-sm text-slate-500 mb-5">
          Email verification is mandatory. Phone verification code flow is implemented in backend and currently disabled by configuration.
        </p>
        ...
        {!awaitingEmailVerification && (
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <input name="branch" value={formData.branch} onChange={handleChange} placeholder="Branch" className="border rounded-lg px-3 py-2" />
            <input name="graduationYear" value={formData.graduationYear} onChange={handleChange} placeholder="Graduation Year" className="border rounded-lg px-3 py-2" />
            {isStudent && <input name="yearOfStudy" value={formData.yearOfStudy} onChange={handleChange} placeholder="Year Of Study" className="border rounded-lg px-3 py-2" />}
            {isAlumni && <input name="currentCompany" value={formData.currentCompany} onChange={handleChange} placeholder="Current Company" className="border rounded-lg px-3 py-2" />}
            {isAlumni && <input name="jobTitle" value={formData.jobTitle} onChange={handleChange} placeholder="Job Title" className="border rounded-lg px-3 py-2" />}
            <input name="location" value={formData.location} onChange={handleChange} placeholder="Location" className="md:col-span-2 border rounded-lg px-3 py-2" />
            <input name="headline" value={formData.headline} onChange={handleChange} placeholder="Headline" className="md:col-span-2 border rounded-lg px-3 py-2" />
            <textarea name="bio" value={formData.bio} onChange={handleChange} rows={3} placeholder="Short Bio" className="md:col-span-2 border rounded-lg px-3 py-2" />
            <input name="skillsText" value={formData.skillsText} onChange={handleChange} placeholder="Skills (comma separated)" className="md:col-span-2 border rounded-lg px-3 py-2" />
            <input name="interestsText" value={formData.interestsText} onChange={handleChange} placeholder="Interests (comma separated)" className="md:col-span-2 border rounded-lg px-3 py-2" />
            <input name="achievementsText" value={formData.achievementsText} onChange={handleChange} placeholder="Achievements (comma separated)" className="md:col-span-2 border rounded-lg px-3 py-2" />
